        ...
        <div class="container">
            <div class="col-2">
                <form method="POST" action="">
                    ...
                    <?php
                    $qt = mysqli_query($conn, "SELECT COUNT(*) AS totalLikes FROM tb_like WHERE image_id=" . $_GET['id']);
        
                    if (mysqli_num_rows($qt) > 0) {
                        while ($q = mysqli_fetch_array($qt)) {
                    ?>
                            <button name="suka" type="submit" class="like">Like <?php echo $q['totalLikes']; ?> </button><br />
                        <?php
                        }
                    } else {
                        ?>
                ...
                if (isset($_POST['suka'])) {
                    ...
                    if (mysqli_num_rows($cekk) > 0) {
                        ...
                        echo "sukai";
                        $que = mysqli_query($conn, "INSERT INTO `tb_like` (`admin_name`, `image_id`, `suka`) VALUES ('" . $adname . "', '" . $gam . "', '1')");

                        if($que){
                            echo '<script>alert("Tambah Like berhasil")</script>';
                        }else{
                    ...
                    echo '<script>window.location="detail-image.php?id='.$_GET["id"].'"</script>'; // Redirect to another page if needed
                }
                ?>
            ...
            <?php 
            $image_id = $_GET['id'];
            $komen = $conn->query("SELECT * FROM komentar_foto where image_id = $image_id");
            
            while ($k = $komen->fetch_array()) { 
            ?>
                <div style="border-bottom: 1px black solid; padding: 20px 0;">
                    <p>
                        <?= $k['isi_komentar'] ?>
                    </p>
                    <span>@<?= $k['admin_name'] ?></span>
                </div>
            <?php
            }
            ?>
